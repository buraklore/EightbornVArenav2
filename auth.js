...
function renderNav(){
  document.getElementById('main-nav').innerHTML='<div class="ni">'+
    ...
    (curUser?
      '<div style="display:flex;align-items:center;gap:12px;padding:10px 18px;border-radius:14px;background:var(--bg3)"><div onclick="go(\'profile\')" style="width:40px;height:40px;border-radius:10px;background:#8b5cf620;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:700;color:var(--v);cursor:pointer">'+esc(curUser.username)[0].toUpperCase()+'</div><div onclick="go(\'profile\')" style="cursor:pointer"><div style="font-size:16px;font-weight:600">'+esc(curUser.username)+'</div><div style="font-size:13px;color:var(--m)">Puan: '+curUser.best_score+'</div></div><button class="btn bg" style="padding:6px 14px;font-size:14px;margin-left:8px;color:var(--pk);border:1px solid var(--pk);border-radius:8px" onclick="doLogout()">Çıkış</button></div>'
    :
